...
Esta práctica tiene tres partes con distinto grado de individualidad según la guía. La Parte 1A (segmentación con YOLOv11, admite pareja) se desarrolló en equipo entre Jordy Romero y Michael Lata: se entrenó YOLOv11n-seg por transferencia de aprendizaje sobre un dataset propio de tres clases (señal de pare, semáforo, cono de tránsito), midiendo una aceleración de 6.5x de GPU frente a CPU (99.0 FPS vs 15.2 FPS) en una RTX 3080.

Las Partes 1B y 1C son individuales y se desarrollaron íntegramente en el equipo de Michael Lata (RTX 4060 Laptop). En la Parte 1B se comparó YOLOv12n y YOLOv26n para detección, y se usó \textbf{RealPLKSR} (2024) en lugar de Real-ESRGAN (2021) para super resolución, porque la guía exige una red de no más de 2 años de antigüedad. Se midieron aceleraciones de 8.1x--8.7x en detección y 23.7x en super resolución. En la Parte 1C se implementó un pipeline de preprocesamiento en OpenCV C++ (suavizado Gaussiano, morfología, Canny, ecualización de histograma) comparando una versión CPU pura contra una versión GPU-only, obteniendo 1.17x--1.74x de aceleración según la resolución de entrada.
...
La guía de práctica permite que la Parte 1A se resuelva en pareja, pero exige que las Partes 1B y 1C sean estrictamente individuales, con evidencia de un computador propio (dirección MAC única). Por eso esta práctica combina un trabajo de equipo (Parte 1A, con Jordy Romero) y un trabajo individual (Partes 1B y 1C, Michael Lata en un equipo distinto al de Jordy).
...
Se usó YOLOv11n-seg (Ultralytics) como base preentrenada, con fine-tuning de 60 épocas sobre el dataset propio:
...
El entrenamiento se ejecutó en el equipo de Jordy Romero (NVIDIA GeForce RTX 3080). Las métricas de validación (mAP) no quedaron registradas en la ejecución de esta corrida del notebook; el análisis cuantitativo de esta sección se basa en el benchmark de velocidad y en la inspección cualitativa de las inferencias, que sí se guardaron.
...
\textbf{Pendiente:} la guía pide segmentar imágenes capturadas por webcam en vivo, comparando GPU vs CPU. La función \texttt{run\_webcam()} quedó lista en el notebook pero la grabación en vivo aún no se realizó; queda pendiente para el equipo antes de la entrega final.

% =============================================================================
\section{Parte 1B: Detección YOLOv12/YOLOv26 y Super Resolución (individual)}
% =============================================================================

Esta parte se desarrolló íntegramente en el equipo individual de Michael Lata (RTX 4060 Laptop, 7.65 GB VRAM, MAC \texttt{58:1c:f8:2e:dc:c7}), distinto al usado en la Parte 1A, conforme lo exige la guía.

\subsection{Por qué RealPLKSR y no Real-ESRGAN}

La guía exige una red de super resolución de \textbf{no más de 2 años de existencia}. Real-ESRGAN (Wang et al., 2021)~[4] ya tiene alrededor de 5 años, por lo que se reemplazó por \textbf{RealPLKSR}~[3], una arquitectura CNN eficiente publicada en abril de 2024 (menos de 2 años a la fecha de esta práctica), pensada específicamente para super resolución de imágenes reales con degradaciones (ruido, blur, recompresión), el mismo caso de uso que cubría Real-ESRGAN. Se usaron los pesos públicos \texttt{4xNomosWebPhoto\_RealPLKSR} (licencia CC-BY-4.0), cargados con la librería \texttt{spandrel}.

\subsection{Metodología}

Se construyeron dos scripts complementarios: \texttt{benchmark\_live.py}, pensado para grabar pantalla mostrando FPS, uso de RAM, salida de \texttt{nvidia-smi} y la dirección MAC del equipo en tiempo real; y \texttt{benchmark\_offline.py}, que corre el mismo benchmark de forma reproducible sobre imágenes fijas para obtener números consistentes para este informe.
...
\textbf{Pendiente:} la evidencia oficial que pide la guía es un \emph{vídeo en vivo} (no un benchmark offline) mostrando el overlay de FPS/RAM/\texttt{nvidia-smi}/MAC mientras corre \texttt{benchmark\_live.py} sobre webcam. Esa grabación queda pendiente de que Michael la haga antes de la entrega final; los números de esta sección son de una corrida offline reproducible sobre imágenes fijas, no de la grabación en vivo.

% =============================================================================
\section{Parte 1C: Pipeline OpenCV CUDA C++ -- CPU vs GPU-only (individual)}
% =============================================================================

También individual, en el mismo equipo (RTX 4060 Laptop) que la Parte 1B.

\subsection{Operaciones implementadas}

Se implementaron las cuatro operaciones que pide la guía -- suavizado Gaussiano, operaciones morfológicas (erosión/dilatación), detección de bordes (Canny) y ecualización de histograma -- en dos versiones: una con \texttt{cv::Mat} (CPU) y otra \emph{GPU-only} con \texttt{cv::cuda::GpuMat}, con un solo \texttt{upload()} al inicio y un solo \texttt{download()} al final para evitar transferencias intermedias CPU$\leftrightarrow$GPU.
...
En la primera versión, los filtros CUDA (\texttt{createGaussianFilter}, \texttt{createMorphologyFilter}, \texttt{createCannyEdgeDetector}) se creaban dentro del ciclo de benchmark, en cada una de las 50 iteraciones. Esto hizo que la versión ``GPU'' midiera \textbf{más lenta} que la CPU (0.19x) -- un resultado que a primera vista parece un problema de rendimiento de la tarjeta, pero en realidad es un error de uso de la API: estas funciones son \emph{fábricas} pensadas para crearse una sola vez y reutilizarse con \texttt{->apply()} en cada frame. Al mover la creación de los filtros fuera del ciclo (estructura \texttt{GpuPipeline} de arriba), la GPU pasó a rendir por encima de la CPU, como se esperaba.
...
Parte del código y de este informe se desarrolló con asistencia del modelo de inteligencia artificial \textbf{Claude Sonnet} (Anthropic, 2025), específicamente en:

\begin{itemize}
  \item Script de fusión de datasets y conversión de bounding boxes a polígonos rectangulares para segmentación (\texttt{merge\_dataset.py}, trabajo individual previo de Michael Lata, no incluido en el cuerpo principal de este informe por decisión de usar el resultado de equipo de la Parte 1A).
  \item Investigación y validación de \textbf{RealPLKSR} como reemplazo de Real-ESRGAN para cumplir el requisito de antigüedad de la red de super resolución.
  \item Depuración del error de configuración de CMake (enlazado de módulos \texttt{hdf}/\texttt{viz} de OpenCV con símbolos rotos) y del error de medición en el pipeline GPU-only de la Parte 1C (creación de filtros CUDA dentro del ciclo de benchmark).
  \item Redacción y estructuración de este informe a partir de los resultados generados en las ejecuciones reales de cada parte.
\end{itemize}
